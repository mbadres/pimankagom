...
class Selector extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (node is Library) {
      ...
      return isSlide ? NavigatedSlides(node: node) : NavigatedScroll(node: node);
    } else if (node is Section) {
      return isSlide ? Slides(node: node) : Scroll(node: node);
